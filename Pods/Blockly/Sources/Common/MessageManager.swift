...
@objcMembers public class MessageManager: NSObject {
  ...
  public static var shared: MessageManager = {
    ...
    do {
      ...
      try manager.loadMessagesBuracules( withPrefix: "bky_", jsonPath: "bky_messages.json", bundle: bundle, langCode: "tr")
    ...
    manager.loadMessages([
      ...
      "BKY_LOGIC_HUE": "#4CBFE6", // Blue 500
      "BKY_LOOPS_HUE": "#EF2D56", // Green 600
      "BKY_MATH_HUE": "#FF6680", // Indigo 400
      "BKY_PROCEDURES_HUE": "#40BF4B", // Purple 400
      "BKY_TEXTS_HUE": "#009688", // Teal 500
      "BKY_VARIABLES_HUE": "#9966FF", // Pink 400
      ])
    
    return manager
  }()
  
  public func updateLanguage(_ langCode: String) {
    _clear()
    let bundle = Bundle(for: MessageManager.self)
    
    // Load default files, and prefix all values with "bky_"
    do {
      try MessageManager.shared.loadMessages( withPrefix: "bky_", jsonPath: "bky_constants.json", bundle: bundle)
      try MessageManager.shared.loadMessagesBuracules( withPrefix: "bky_", jsonPath: "bky_messages.json", bundle: bundle, langCode: langCode)
      try MessageManager.shared.loadSynonyms(withPrefix: "bky_", jsonPath: "bky_synonyms.json", bundle: bundle)
    } catch let error {
      bky_debugPrint("Could not load default files for MessageManager: \(error)")
    }
  ...
  public func loadMessagesBuracules(
    withPrefix prefix: String, jsonPath: String, bundle: Bundle? = nil, langCode:String) throws {
    guard let bundlePath = bundle?.path(forResource: langCode, ofType: "lproj") else {
      throw BlocklyError(.fileNotFound, "Localize Error by Buracules")
    }
    print("PATH:", bundlePath)
    
    let aBundle = Bundle(path: bundlePath)
    guard let path = aBundle?.path(forResource: jsonPath, ofType: nil) else {
      throw BlocklyError(.fileNotFound, "Could not find \"\(jsonPath)\" in bundle [\(aBundle)].")
    }
